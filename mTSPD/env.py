...
class Environment():
    ...
    def reset(self, coordinates=None, route=None):
        if coordinates is not None:
            self.coordinates = torch.cat([coordinates[:, :2], torch.zeros((50, 1), device=coordinates.device)], dim=1)
        
        if route is not None:
            # 计算distances和total_distance（模拟LKH输出）
            distances = []
            for i in range(len(route) - 1):
                dist = self.calculate_distance(route[i], route[i + 1])
                distances.append(dist)
            total_distance = sum(distances)
            self.result = {'result': route, 'distances': distances, 'total_distance': total_distance}
    ...
    def process_result(self, result):
        ...
        '''
        route_len = len(result['result']) - 1  # 路由长度（边数）
        self.edge_index = torch.zeros((2, route_len * 2), dtype=torch.long) # 无向图，边是双向的
        self.edge_attr = torch.zeros((route_len * 2, 2), dtype=torch.float) # 属性1为距离，属性2为边的种类
        for i in range(route_len): # 构建边
            self.edge_attr[i*2] = torch.tensor([result['distances'][i], COMMON_ROUTE], dtype=torch.float)
            self.edge_attr[i*2+1] = torch.tensor([result['distances'][i], COMMON_ROUTE], dtype=torch.float)
            self.edge_index[0][i*2] = result['result'][i] # 这两行加入了i指向i+1的边
            self.edge_index[1][i*2] = result['result'][i+1]
            self.edge_index[0][i*2+1] = result['result'][i+1] # 这两行加入了i+1指向i的边
            self.edge_index[1][i*2+1] = result['result'][i]
