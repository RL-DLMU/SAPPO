...
NUM_NO_FLY = 5  # 生成禁飞区节点的数量（若用真实数据，可去除）

DATA_PATH = f"data/nodes{NODE_NUM}.pkl"  # 数据集路径
CSV_PATH = f"no-fly/result_info/{'Greedy' if GREEDY else 'Sample'}_{NODE_NUM}nodes_{DECISION_REGION}_{SUB_DECISION_REGION}_result.csv"  # CSV 输出路径

# 模型路径
MODEL_PATH = f"trained_models/best_model_{DECISION_REGION}_{SUB_DECISION_REGION}.pth"

result_save_path = "no-fly/result.svg"
...
def infer(visible=False):
    ...
    for episode in range(max_episodes):
        ...
        if episode % CHANGE_GRAPH == 0:
            ...
            # 随机选择禁飞区节点（排除起点0）
            no_fly_nodes = random.sample(range(1, NODE_NUM), k=NUM_NO_FLY)
            obs, original_time = env.reset(data, no_fly=no_fly_nodes)
        else:
