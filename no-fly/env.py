...
class Environment():
    def __init__(self, coord_max, node_num, device, decision_region, sub_decision_region):
        ...
        self.ax = None
        self.time_step = 0
        self.no_fly = torch.zeros(self.node_num, dtype=torch.bool).to(self.device)

    def reset(self, data=None, no_fly=None):
        if data != None:
            ...
            if no_fly is not None:
                self.no_fly = torch.zeros(self.node_num, dtype=torch.bool).to(self.device)
                for nid in no_fly:
                    self.no_fly[nid] = True
        else:
            self.coordinates[:, 2] = 0  # Reset visited status for reused graph
        ...
        for i in range(self.decision_region): # 使用决策索引和决策mask来处理邻近终止情况下的观测数据
            ...
            # Apply no-fly mask only for potential intermediate nodes
            if self.no_fly[decision_node[i]] and 0 < i < self.decision_region - 1:
                decision_mask[i] = 0
    ...
    def draw(self):
        ...
        # 添加点到绘图中
        colors = ['red'] * self.node_num
        for i in range(self.node_num):
            if self.no_fly[i]:
                colors[i] = 'black'
        colors[0] = 'yellow'
        self.ax.scatter(city_x, city_y, color=colors, linewidths=0.5)
        # 添加每个点的ID作为标签
    ...
    def save_result_to_img(self, file_path='SAPPO/result.svg'):
        ...
        # 创建线段集合
        line_segments1 = LineCollection(line_common, colors='blue', linewidths=1.5,label='common route')
        line_segments2 = LineCollection(line_drone, colors='red', linewidths=1.5,label='drone route')
        line_segments3 = LineCollection(line_truck, colors='green', linewidths=1.5,label='truck route')
        ...
        for i in range(self.node_num):
            if self.no_fly[i]:
                colors[i] = 'black'
        colors[0] = 'yellow'
        ax.scatter(city_x, city_y, color=colors, linewidths=0.5)
        
        # 设置坐标范围
        ax.set_xlim(0, self.coord_max)
        ax.set_ylim(0, self.coord_max)

        ax.set_title(f'No Fly Zone')
