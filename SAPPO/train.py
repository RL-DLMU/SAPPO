...
result_save_path = "SAPPO/result.svg"
DATA_PATH = f"data/nodes{NODE_NUM}.pkl"

def load_data(path, offset=0, num_samples=10000):
...
def train(visible=False):
    ...
    best_model = 1000
    loaded_data = load_data(DATA_PATH, 0, 10)
    for episode in range(MAX_EPISODES):
        s_t = time.time()
        # 重置环境，获取初始观测和图数据
        if episode % CHANGE_GRAPH == 0:
            # data = COORD_MAX * torch.rand((NODE_NUM, 2), dtype=torch.float) # 生成node_num个坐标，两列分别为x与y的值，坐标范围在[0, COORD_MAX]
            data = loaded_data[0]
        ...
        if total_time / original_time < best_model:
            ...
            torch.save(agent.state_dict(), f'trained_models/{NODE_NUM}nodes_best_model_{DECISION_REGION}_{SUB_DECISION_REGION}.pth')
